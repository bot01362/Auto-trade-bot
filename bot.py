...
-----------------------------
Price source: Coinbase public spot price endpoint (ETC-USD, no API key required)
...
Rebuilt using the same structure as the current BTC and Gold bots:
  - MA spread threshold: 0.08%
  - RSI bands: 58 (buy ceiling) / 42 (sell floor)
...
logger = logging.getLogger("etc_bot")
...
# Config
# ---------------------------------------------------------------------------
COINBASE_URL = "https://api.coinbase.com/v2/prices/ETC-USD/spot"
...
# --- Signal confidence filtering (STRICTER than before) ---
MIN_MA_SPREAD_PCT = 0.08      # lowered from 0.15 — 0.15 was filtering out real moves in flat markets
RSI_BUY_CEILING = 58          # widened slightly from 55 — small extra room below midline for a BUY
RSI_SELL_FLOOR = 42           # widened slightly from 45 — small extra room above midline for a SELL
CONFIRMATION_STREAK = 3       # lowered from 5 — only needs 3 min sustained trend instead of 5
...
@app.route("/")
def home():
    return "ETC/USDT bot is alive.", 200
...
# ---------------------------------------------------------------------------
def fetch_price():
    """Fetch the latest ETC/USD spot price from Coinbase. Returns float or None.
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if price is not None:
        ...
        await update.message.reply_text(
            "⚠️ Couldn't fetch the current ETC price. Try again shortly."
        )
...
async def price_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    price = fetch_price()
    if price is not None:
        await update.message.reply_text(f"🔷 ETC/USDT: ${price:,.2f}")
    else:
        await update.message.reply_text("⚠️ Couldn't fetch the current ETC price.")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "refresh":
        ...
        if price is not None:
            ...
            await query.edit_message_text("⚠️ Couldn't fetch the current ETC price.")
...
def main():
    ...
    logger.info("ETC/USDT bot starting...")
